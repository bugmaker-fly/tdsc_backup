...
def main():
    ...
    result_base = '/home/rtfeng/fei/'+pn+'/result2_b'
    model_begin_l = ['CDT_LMT_8']
    acc_begin_l = [0.9]
    ...
    for i, model_folder in enumerate(model_begin_l):
        ...
        model_path = result_base + '/' + model_folder + '/model_4'
        print(model_path)
        model_file = glob.glob(model_path + '/*.pkl')
        m_len = len(model_file)
        print(m_len)
        f_batch = open('/home/rtfeng/fei/qiao_newdata2/batch/CDT_LMT_8/batch_continue.csv', 'w')
        f_sou = open('/home/rtfeng/fei/qiao_newdata2/batch/CDT_LMT_8/batch.csv','r',encoding='utf-8')
        for sou_line in f_sou.readlines():
            for j, str1 in enumerate(sou_line.strip().split(',')):
                f_batch.write(str1) #-------------------------------- _0 ---------------------------------
                if j != len(sou_line.strip().split(','))-1:
                    f_batch.write(',')
            f_batch.write('\n')
        ...
        if True:
            #print('--------------------here--------------------')
            del_path = result_base + '/' + model_folder + '/best_train_set_now_4.csv'
            ...
            save_path = '/home/rtfeng/fei/'+pn+'/result_b/' + model_folder  # 存放结果 -------------------------------改了这里----------------------
            mkdir(save_path)
            #model_path = save_path + '/model'
            #mkdir(model_path)
            f_detail = open(save_path + '/detail_continue.csv', 'w')
