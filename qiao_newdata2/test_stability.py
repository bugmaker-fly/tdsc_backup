...
def get_model(train_path, test_path1, test_path2, args):
    ...
    #print(x_test1)
    #保存模型
    model_path = '/home/rtfeng/fei/'+pn+'/test_stability_result/'+save_dir+'/model' + '/model'  + '.pkl'
    ...
    hammingloss = hamming_loss(y_true=y_test1, y_pred=y_pred1)
    oneerror = zero_one_loss(y_true=y_test1, y_pred=y_pred1)
    instancef1 = f1_score(y_true=y_test1, y_pred=y_pred1, average='samples')
    label_acc = get_label_acc(y_pred1, y_test1)
    return acc1, hammingloss, oneerror, instancef1,label_acc, model_path
...
def main():
    ...
    f_re = open('/home/rtfeng/fei/'+pn+'/test_stability_result/'+save_dir+'/result.csv','w')
    f_re.write('data,model,sample-acc,hamming-loss,one-error,instance-f1\n')
    f_re.write('CDN_REPTree_8,CC_RandomForest_32,')

    result_path = '/home/rtfeng/fei/'+pn+'/test_stability_result'
    f_feature = result_path + '/final_feature_500.txt'
    test_path1 = result_path + '/train_and_test.arff'
    test_path2 = result_path + '/manual_test.arff'
    train_temp_csv = result_path +'/'+save_dir+'/best_train_set_now_4.csv'
    del_csv = result_path + '/train_and_test.csv'
    train_csv = result_path + '/'+save_dir+'/train_csv.csv'
    len = del_input_4000(train_temp_csv, del_csv, train_csv)

    train_arff = csv2arff(train_csv, f_feature, feature_num+label_num)
    acc1, hamming_loss, one_error, instance_f1, label_accs,model = get_model(train_arff, test_path1,test_path2, args)
    f_re.write(str(acc1))
    f_re.write(',')
    f_re.write(str(hamming_loss))
    f_re.write(',')
    f_re.write(str(one_error))
    f_re.write(',')
    f_re.write(str(instance_f1))
    f_re.write('\n')
    f_re.write('label acc:\n')
    for i,label_acc in enumerate(label_accs):
        f_re.write(str(label_acc))
        if i != 5:
            f_re.write(',')
    f_re.write('\n')
    ...
    print(f'one error:{one_error}')
    print(f'instance f1:{instance_f1}')
